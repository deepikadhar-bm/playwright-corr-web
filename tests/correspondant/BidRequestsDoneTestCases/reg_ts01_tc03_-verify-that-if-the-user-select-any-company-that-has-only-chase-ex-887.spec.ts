...
const TC_TITLE = 'Verify that if the user select any company that has only chase execution type, then he should be able to select the valiues from the chase exe type only and standard should be disbaled.';
...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test(`${TC_ID}_${TC_TITLE}`, async ({ page }) => {
    ...
    try {
      ...
      // ── Step 5: Set Standard Flow OFF and Chase Direct ON ─────────────────
      log.step('Setting Standard Flow OFF and Chase Direct ON');
      try {
        ...
        log.stepPass('Standard Flow set to OFF and Chase Direct set to ON successfully');
      } catch (e) {
        await log.stepFail(page, 'Setting Standard Flow OFF or Chase Direct ON failed');
        throw e;
      }
      ...
      try {
        ...
        await expect(customerPermissionPage.Chasedirect_Execution_PermissionAllowed(vars["CompanyName_CustomerPermissions"])).toContainText("Allowed");
        log.stepPass(`Customer permission verified - Standard: Disabled, Chase Direct: Allowed for company: ${vars["CompanyName_CustomerPermissions"]}`);
      } catch (e) {
      ...
      // ── Step 10: Verify Chase Direct enabled and Standard disabled ────────
      log.step('Verifying Chase Direct is enabled and Standard Execution is disabled');
      try {
        await expect(chaseDirectPage.ChaseDirect_Checkbox).toBeEnabled();
        await chaseDirectPage.ChaseDirect_Checkbox.check();
        ...
        log.stepPass('Chase Direct is enabled and Standard Execution is visible as expected');
      } catch (e) {
        await log.stepFail(page, 'Verifying Chase Direct or Standard Execution state failed');
        throw e;
      }
