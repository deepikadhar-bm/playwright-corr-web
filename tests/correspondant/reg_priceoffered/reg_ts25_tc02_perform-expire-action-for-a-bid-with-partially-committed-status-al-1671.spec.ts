...
const TC_ID = 'REG_TS25_TC02';
const TC_TITLE = 'Perform expire action for a bid with partially committed status. Also verify that user should be able to update back to partially committed';
...
test.describe('Unassigned', () => {
  ...
  test(`${TC_ID} - ${TC_TITLE}`, async ({ page }) => {
    ...
    try {
      ...
      log.step('Navigating to Price Offered, committing a fresh loan and verifying Partially Committed status');
      try {
        ...
        log.stepFail(page, 'Failed to commit loan or verify Partially Committed status');
        throw e;
      }

      log.step('Capturing company name and calculating total unlocked loan amount');
      try {
        ...
        log.stepPass('Total unlocked loan amount calculated: ' + vars['TotalLoanAmount']);
      } catch (e) {
        log.stepFail(page, 'Failed to calculate total unlocked loan amount');
        throw e;
      }
      ...
      try {
        ...
        log.stepPass('Expire action performed and popup data verified successfully');
      } catch (e) {
        log.stepFail(page, 'Failed to perform expire action or verify expire popup data');
        throw e;
      }
      ...
      try {
        ...
        log.stepFail(page, 'Failed to verify bid status as Expired');
        throw e;
      }

      log.step('Opening expired bid, verifying Get Price button and calculating total loan amount from details screen');
      try {
        await priceOfferedPage.Bid_Req_IdPrice_Offered_Page(vars["BidReqIdPriceOffered"]).click();
        ...
        log.stepPass('Get Price button verified and total loan amount calculated: ' + vars['TotalLoanAmount']);
      } catch (e) {
        log.stepFail(page, 'Failed to verify Get Price button or calculate total loan amount from details screen');
        throw e;
      }

      log.step('Performing change status action and verifying change status popup data');
      try {
        await priceOfferedPage.BackTo_PriceofferedPage.click();
        ...
        log.stepPass('Change status popup data verified and status change confirmed');
      } catch (e) {
        log.stepFail(page, 'Failed to verify change status popup data or perform status change');
        throw e;
      }

      log.step('Reloading page and verifying bid status is back to Partially Committed');
      try {
        await page.reload();
        await spinnerPage.Spinner.waitFor({ state: 'hidden' });
        await bidRequestsPage.Search_by_Bid_Request_ID_Field.clear();
