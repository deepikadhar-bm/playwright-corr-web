  ...
  test(`${TC_ID} - ${TC_TITLE}`, async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        log.stepPass("Step 3 passed: All fields selected using Select All checkbox");
      } catch (error) {
        log.stepFail(page, "Step 3 failed: Failed to select all fields");
        throw error;
      }

      log.step("Step 4: Deselect all fields using Select All checkbox");
      try {
        await correspondentPortalPage.Select_All_Checkbox.uncheck();
        log.stepPass("Step 4 passed: All fields deselected successfully");
      } catch (error) {
        log.stepFail(page, "Step 4 failed: Failed to deselect all fields");
        throw error;
      }
      ...
      try {
        ...
        log.stepPass("Step 5 passed: All header fields verified successfully after deselection");
      } catch (error) {
        log.stepFail(page, "Step 5 failed: Header field verification failed after deselection");
        throw error;
      }
