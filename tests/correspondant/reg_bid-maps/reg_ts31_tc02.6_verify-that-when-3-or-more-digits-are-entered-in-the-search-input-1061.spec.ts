...
test.describe('Unassigned', () => {
  ...
  "UniqueRuleNameSearch": "TsSearchUniqueRuleName"
    ...
    await statusInactive2Page.SearchFilter_Fields.fill(testData["UniqueWhenBidFieldSearch"]);
    ...
    await expect(bidmapsPage.Rules_Conditions_SearchFilter_Value_Text).toContainText(testData["UniqueWhenBidFieldSearch"]);
    ...
    await spinnerPage.Spinner.waitFor({ state: 'hidden' });
    await expect(whenBidFieldPage.When_Bid_Field_Dropdown).toContainText(testData["UniqueWhenBidFieldSearch"]);
    await expect(rulesAndActionsPage.Bid_Enumerated_Tape_Value_Dropdown).toContainText(testData["UniqueBidEnumTapeSearch"]);
