...
const TC_TITLE = 'Verify that when the user performs the "Save Draft and Exit" action on each screen, a draft version is saved, and the user is redirected to the bid map list screen.[Rules and Actions]';
...
test.describe('REG_Bid Maps', () => {
  ...
  test(`${TC_ID} - ${TC_TITLE}`, async ({ page }) => {
    ...
    try {
      ...
      log.step('Save Draft and Exit, then verify draft is saved on navigation back');
      try {
        ...
        await expect(chaseFieldName2Page.Add_Actions_Chase_Field_Name).toContainText(vars["ChaseFiledNameonAddActions"]);
        await expect(chaseValuePage.Add_Actions_Chase_Value).toContainText(vars["ChasevalueOnAddActions"]);
        log.stepPass('Draft verified: Rule Name, Condition, and Action fields match saved values');
      } catch (e) {
        await log.stepFail(page, 'Draft verification failed after Save Draft and Exit');
        throw e;
      }
      ...
      log.step('Save Draft and Exit from Rules and Actions screen');
      try {
        await page.waitForTimeout(3000);
        await saveDraftExitButtonPage.Save_Draft_Exit_Button.click();
        await saveDraftExitButtonPage.Save_Draft_Exit_Button.waitFor({ state: 'hidden' });
        await spinnerPage.Spinner.waitFor({ state: 'hidden' });
        log.stepPass('Save Draft and Exit performed successfully');
      } catch (e) {
        await log.stepFail(page, 'Failed to Save Draft and Exit');
        throw e;
      }

      log.step('Navigate back to Bid Map and verify draft is accessible');
      try {
        ...
        await correspondentPortalPage.Yes_Proceed_Button.click();
        await spinnerPage.Spinner.waitFor({ state: 'hidden' });
        log.stepPass('Navigated back to Rules and Actions via Bid Map list');
      } catch (e) {
        await log.stepFail(page, 'Failed to navigate back to Rules and Actions from Bid Map list');
        throw e;
      }

      log.step('Verify all edited rule blocks are preserved after Save Draft and Exit');
      try {
        ...
        await log.stepFail(page, 'Rule block verification failed after navigating back');
        throw e;
      }

      log.step('Delete rule and verify deletion is saved via Save Draft and Exit');
      try {
        ...
        await expect(rulesAndActionsPage.Duplicated_Block).not.toBeVisible();
        log.stepPass('Deleted rule is not visible; duplicated block is still visible');
      } catch (e) {
