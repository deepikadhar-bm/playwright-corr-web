...
test.describe('REG_General Settings', () => {
  ...
  test('REG_TS06_TC03_Early Config - Verify the Select data range calnder functionality ', async ({ page }) => {
    ...
    //   return fmt.replace('yyyy', p.year || '').replace('yy', (p.year||'').slice(-2)).replace('MM', p.month || '').replace('dd', p.day || '').replace('HH', String(d.getHours()).padStart(2,'0')).replace('hh', p.hour || '').replace('mm', p.minute || '').replace('ss', p.second || '').replace('a', p.dayPeriod || '').replace(/M(?!M)/g, String(parseInt(p.month||'0'))).replace(/d(?!d)/g, String(parseInt(p.day||'0'))).replace(/h(?!h)/g, String(parseInt(p.hour||'0')));
    // })();
    while (await earlyConfigPage.Early_Conf_Del_Button_other_than_today.isVisible()) {
      await earlyConfigPage.Early_Conf_Del_Button_other_than_today.hover();
      await expect(page.getByText("Delete")).toBeVisible();
      await earlyConfigPage.Early_Conf_Del_Button_other_than_today.click();
      await correspondentPortalPage.Yes_Delete_ButtonEarly_config.click();
      await spinnerPage.Spinner.waitFor({ state: 'hidden' });
      // [DISABLED] Wait until the element Early Conf Del Button other than today is not visible
      // await earlyConfigPage.Early_Conf_Del_Button_other_than_today.waitFor({ state: 'hidden' });
    }
    await expect(earlyConfigPage.Early_Conf_Del_Button_other_than_today).toBeVisible();
