...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  "Bid Enum Check(Error Description)": "enum field occupancy type is missing value or value is not in valid list [primary residence, secondary residence, investment (noo)]. path: search.criteria.propertyuse is missing value"
    ...
    await correspondentPortalPage.Upload_File.setInputFiles(path.resolve(__dirname, 'test-data', "GEO_scenario_1.1_happy_flow.xlsx"));
    ...
    await expect(bidRequestDetailsPage.Errors_Column).toContainText(testData["Geo Coding happy flow (Errors Column)"]);
    await expect(bidRequestPage.Error_Description_ColumnBid_request_details).toContainText(testData["Geo Coding happy flow(Error Description)"]);
    await expect(bidRequestDetailsPage.Loan_Status_Column).toContainText(testData["Geo Coding happy flow(Loan Status)"]);
