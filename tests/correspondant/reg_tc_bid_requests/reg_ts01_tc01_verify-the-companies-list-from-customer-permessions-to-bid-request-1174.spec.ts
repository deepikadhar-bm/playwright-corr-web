...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  "UniqueRuleNameSearch": "TsSearchUniqueRuleName"
    ...
    for (let dataIdx = -1; dataIdx <= parseInt(vars["TotalCompaniesCount(CustomerPermission)"]); dataIdx++) {
      vars["IndividualCompanyInPermissions"] = await chaseFieldNamePage.Individual_Company_in_Customer_permission.textContent() || '';
    ...
    for (let dataIdx = -1; dataIdx <= parseInt(vars["TotalCompaniesCount(CustomerPermission)"]); dataIdx++) {
      ...
      await expect(addHeaderPage.Individual_Company_NameBidrequest_Dropdown).toContainText(vars["IndividualCompanyname"]);
    }
    ...
    vars["FirstCompanyname"] = await chaseFieldNamePage.Selecting_first_company_in_the_Dropdown.inputValue() || '';
    { const _m = String(vars["FirstCompanyname"]).match(/.{3,4}/); if (_m) vars["FirstCompanyname"] = _m[0]; } 
    await bidRequestPage.Bid_Mapping_IdSearch_Input_box.fill(vars["FirstCompany_name"]);
    await expect(p15ActivePage.Searched_Company_name).toBeVisible();
    ...
    for (let dataIdx = 1; dataIdx <= 1; dataIdx++) {
      ...
      await expect(statusInactive4Page.Entered_Company_name).toBeVisible();
    }
  });
});
