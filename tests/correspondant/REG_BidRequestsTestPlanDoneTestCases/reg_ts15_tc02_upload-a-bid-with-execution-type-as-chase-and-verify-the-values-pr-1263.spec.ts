...
const TC_TITLE = 'Upload a bid with execution type as chase and verify the values present in the accordian table';
...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS15_TC02_Upload a bid with execution type as chase and verify the values present in the accordian table', async ({ page }) => {
    ...
    try {
      ...
      log.step('Logging in to Correspondent Portal and performing initial navigation steps');
      try {
        ...
        log.info('Login and initial navigation completed successfully');
      ...
      // await stepGroups.stepGroup_Deleting_Early_Config_Report_If_Present(page, vars);
      // await stepGroups.stepGroup_Navigating_to_Upload_New_Bid_Request(page, vars);
      //  await selectDropdownPage.Select_Company_In_BidRequest.waitFor({ state: 'visible' });
      //  await stepGroups.stepGroup_Upload_bid_request_with_execution_type_chase(page, vars);
      ...
      // ── Compute current EST date and time ─────────────────────────────────
      log.step('Computing current EST date and time');
      try {
        vars["CurrentESTDate"] = (() => {
          const d = new Date();
        ...
        log.info(`Current EST HourMin: ${vars["CurrentEstHourMin"]}, TimeUnit: ${vars["CurrentEstTimeUnit"]}`);

        log.stepPass('Current EST date and time computed successfully');
      } catch (e) {
        await log.stepFail(page, 'Computing current EST date and time failed');
        throw e;
      }
      ...
      try {
        await expect(bidRequestDetailsPage.Execution_Type_from_Details_table1).toContainText(appconstants.ChaseDirectExecutionTableHeader);
        log.info(`Execution type verified: ${appconstants.ChaseDirectExecutionTableHeader}`);
      ...
      try {
        ...
        if (await bidRequestDetailsPage.Errored_Loans_Header1.isVisible()) /* Element Errored Loans Count from Rows is visible */ {
