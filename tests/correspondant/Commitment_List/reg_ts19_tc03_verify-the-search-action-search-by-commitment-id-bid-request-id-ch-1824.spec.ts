...
const TC_TITLE = 'Verify the search action, Search by Commitment ID, Bid Request ID, Chase Loan Number and the Correspondent Loan Number for full input';
...
test.describe('Commitment List - TS_1', () => {
  ...
  test(`${TC_ID} - ${TC_TITLE}`, async ({ page }) => {
    ...
    try {
      ...
      log.step('Navigate to Commitment List and capture search field values');
      try {
        ...
        log.stepPass('Navigated to Commitment List and captured all search field values');
      } catch (e) {
        await log.stepFail(page, 'Failed to navigate to Commitment List or capture search field values');
        throw e;
      }
