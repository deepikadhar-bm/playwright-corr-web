...
test.describe('Commitment List Suite', () => {
  ...
  test('REG_TS18_TC02_Apply filter for current day/particular day and verify the Export functionality', async ({ page }) => {
    ...
    log.info("Export excel headers verification with UI")
    ...
    while (parseFloat(String(vars["Count"])) <= parseFloat(String(vars["CountOfHeaders"]))) {
      ...
      log.info(vars["IndividualHeadersUI"]+"contains"+vars["IndividualExcelHeaders"])
      vars["Count"] = (parseFloat(String("1")) + parseFloat(String(vars["Count"]))).toFixed(0);
      vars["count"] = (parseFloat(String("1")) + parseFloat(String(vars["count"]))).toFixed(0);
    }
    log.info("Successfully completed testcase-TS18_TC02")
  });
});
});