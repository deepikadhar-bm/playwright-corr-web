...
import { runPrereq_1306 } from './prereq-1306';
...
const TC_ID = 'PREREQ_1406(REG_TS22_TC04_01.1)';
...
export async function runPrereq_1406(page: Page, vars: Record<string, string>): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      await spinnerPage.Spinner.waitFor({ state: 'hidden' });
      log.stepPass('Resubmit for pricing submitted and bid upload progress verified');
    } catch (e) {
