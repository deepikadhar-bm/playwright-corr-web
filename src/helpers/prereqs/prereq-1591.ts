...
const TC_ID = 'PREREQ_1591(REG_TS22_TC03.2)';
...
export async function runPrereq_1591(page: Page, vars: Record<string, string>): Promise<void> {
  ...
  try {
    ...
    log.step('Set Bid Request ID and uncommit existing committed loans');
    try {
    ...
    try {
      ...
      log.stepPass('Price Offered status verified. Chase Direct: ' + vars['BidStatusChasePriceOffered'] + ' | Standard: ' + vars['BidStatusStandardPriceOffered']);
    } catch (e) {
