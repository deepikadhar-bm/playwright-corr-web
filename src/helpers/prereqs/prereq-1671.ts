...
export async function runPrereq_1671(page: Page, vars: Record<string, string>): Promise<void> {
  ...
  try {
    log.step(' Navigate to Price Offered, commit a fresh loan and verify Partially Committed status');
    try {
      ...
      await log.stepFail(page, ' Failed to commit loan or verify Partially Committed status');
      throw e;
    }

    log.step(' Capture company name and calculate total unlocked loan amount');
    try {
      ...
      await log.stepFail(page, ' Failed to calculate total unlocked loan amount');
      throw e;
    }

    log.step(' Perform expire action and verify expire popup data');
    try {
      ...
      await log.stepFail(page, ' Failed to perform expire action or verify expire popup data');
      throw e;
    }

    log.step(' Reload page and verify bid status is Expired');
    try {
      await page.reload();
      await bidRequestsPage.Search_by_Bid_Request_ID_Field.clear();
      ...
      await log.stepFail(page, ' Failed to verify bid status as Expired');
      throw e;
    }

    log.step(' Open expired bid, verify Get Price button and calculate total loan amount');
    try {
      ...
      await log.stepFail(page, ' Failed to verify Get Price button or calculate total loan amount');
      throw e;
    }

    log.step(' Perform change status action and verify change status popup data');
    try {
      await priceOfferedPage.BackTo_PriceofferedPage.click();
      ...
      await log.stepFail(page, ' Failed to verify change status popup data or perform status change');
      throw e;
    }

    log.step(' Reload page and verify bid status is back to Partially Committed');
    try {
      await page.reload();
      await spinnerPage.Spinner.waitFor({ state: 'hidden' });
      await bidRequestsPage.Search_by_Bid_Request_ID_Field.clear();
      ...
      await log.stepFail(page, ' Failed to verify Bid status Partially Committed');
      throw e;
    }
    log.tcEnd('PASS');
  } catch (e) {
