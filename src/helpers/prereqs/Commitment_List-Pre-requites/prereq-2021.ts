...
const TC_ID = 'PREREQ_2021(REG_TS25_TC01)';
...
export async function runPrereq_2021(page: Page, vars: Record<string, string>): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      log.stepPass('Successfully navigated to closed list tab');
    } catch (e) {
      log.stepFail(page, 'Failed to navigate to closed list tab');
      throw e;
    }
