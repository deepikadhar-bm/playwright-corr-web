...
export async function runPrereq_1261(page: Page, vars: Record<string, string>): Promise<void> {
...
const TC_ID = 'PREREQ-1261';
const TC_TITLE = 'Prerequisite for reg_ts24_tc02_03 - Upload a bid request with Next Business Day Queued For status.';
   ...
   log.tcStart(TC_ID, TC_TITLE);
      ...
      try {
        ...
        try {
          ...
          vars["SelectedLockTerm"] = await bidRequestPage.Chase_Direct_DropdownUpload_Bidrequest.evaluate(el => { const s = el as HTMLSelectElement; return s.options[s.selectedIndex]?.text || ''; });
      ...
      try {
        ...
        throw e;
           ...
           await expect(bidRequestDetailsPage.Expected_Execution_Type_Parsed_rowcd(vars["SelectedLockTerm"])).toBeVisible();
        ...
        try {
          ...
          if (footerText?.includes(vars["CurrentEstHourMin"]))/* Verify that the element Footer Submission Date displays text */ {
        ...
        try {
          await expect(bidRequestDetailsPage.Footer_Queued_For_Date).toContainText(vars["NextBusinessDate"]);
          log.info(`Footer Queued For Date contains NextBusinessDate: ${vars["NextBusinessDate"]}`);
