...
export async function runPrereq_1253(page: Page, vars: Record<string, string>): Promise<void> {
  ...
  const TC_ID = 'PREREQ-1253';
  const TC_TITLE = 'Prerequisite for reg_ts24_tc02_02_verify-that-submit-for-pricing-button-should-be-disabled-for-the-status-and-the-Queued-For-Time-text-Should-be-Red-In-colour-for-passed-time';
     ...
     try {
       ...
       try {
         ...
         vars["Time Interval"] = "4";
          log.info(`Set Time Interval: ${vars["Time Interval"]}`);
