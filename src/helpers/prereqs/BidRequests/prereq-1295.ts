...
export async function runPrereq_1295(page: Page, vars: Record<string, string>): Promise<void> {
  ...
  const TC_ID = 'PREREQ-1295';
  const TC_TITLE = 'Setup batch timing, store buffered batch times in test data, navigate to bid requests and resubmit for pricing.';
  ...
  try {
    ...
    try {
      ...
      log.stepPass('Test data and credentials loaded successfully');
    } catch (e) {
      await log.stepFail(page, 'Failed to load test data or credentials');
      throw e;
    }

    // ── Step 2: Login & Navigate to Bulk Batch Timing ────────────────────
    ...
    try {
      ...
      log.info('Batch intervals modified with 5 min prior');
    ...
    // ── Step 3: Capture Buffer Time & Store Buffered Batch Timings ────────
    ...
    try {
      ...
      for (let dataIdx = parseInt(vars["count"]); dataIdx <= parseFloat(String(vars["TotalExtractedBatch"])); dataIdx++) {
        ...
        testDataManager.updatePartialProfileDataByDataIndex(
          'Bid Requests',
          { 'Stored_Text': vars["EachBatchTiming"] },
          dataIdx
        );
        log.info(`Stored_Text updated in test data at index ${dataIdx}: ${vars["EachBatchTiming"]}`);
    ...
    // ── Step 4: Navigate to Bid Requests & Apply Filters ─────────────────
    ...
    // ── Step 5: Open Bid Request & Resubmit for Pricing ──────────────────
