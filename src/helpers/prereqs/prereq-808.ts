...
// }; // Profile: "Bid_Maps", row: 0
  ...
  if (profile && profile.data) {
      const chaseFieldName = profile.data[0]['ChaseFieldNames'];
      vars["ChaseFieldNames"] = chaseFieldName;
  ...
  await correspondentPortalPage.Chase_Field_Name.selectOption({ label: vars["ChaseFieldNames"].trim() });
  ...
  await correspondentPortalPage.Search_Map_Input.last().fill(vars["Search Map Input"]);
