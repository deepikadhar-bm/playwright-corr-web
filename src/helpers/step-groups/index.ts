...
//   await CorrPortalElem.Compare_Button.click();//Create button
//   await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
...
//   await CorrPortalElem.Upload_File.setInputFiles(path.resolve(__dirname, 'test-data', "DeepikaAugBidQA.xlsx"));//uploadFile
...
export async function stepGroup_Creation_Of_Bid_Map_Upto_Header_Mapping(page: import('@playwright/test').Page, vars: Record<string, string>, fileName: string) {
  ...
  Helpers.concatenate(appconstants.Testsigma_, vars["Current Date"], "CreateNewMap"); /* format: MM/dd/yyyy/HH:mm:ss */;
  await CorrPortalElem.Create_New_Map_Field.fill(vars["CreateNewMap"]);
  vars["BidMap"] = await CorrPortalElem.Create_New_Map_Field.inputValue() || '';
  await CorrPortalElem.Compare_Button.click();
  await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
...
export async function stepGroup_Creation_Of_Bid_Map_Upto_Header_Mapping_with_Standard(page: import('@playwright/test').Page, vars: Record<string, string>, fileName: string) {
  ...
  await CorrPortalElem.Required_Company_s_Name_Value(vars["Companyname"]).first().click();
  await CorrPortalElem.Apply_Selected.click();
  await correspondentPortalPage.Execution_Type_Dropdown.selectOption({ label: appconstants.EXECUTION_TYPE_STANDARD });
  await expect(CorrPortalElem.Upload_File).toHaveValue('');
  await expect(page.getByText("Drag and drop files here or click to browse. Allowed formats: .xls,.xlsx,.csv,.txt")).toBeVisible();
  await uploadFile(page, correspondentPortalPage.Upload_File, fileName);
  await CorrPortalElem.Map_Headers_Button.click();
...
export async function stepGroup_Import_Rule_in_Mapping(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  try {
    ...
    vars["Version"] = (await CorrPortalElem.Version.textContent())?.trim() || '';
...
export async function stepGroup_Navigating_to_Bulk_Batch_Timing(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.Administration_Menu.click();
  await CorrPortalElem.General_Settings.first().click();
  await page.waitForLoadState('load');
...
export async function stepGroup_Navigation_to_Customer_Permission(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  try {
    ...
    await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
    await expect(page.getByText("Customer Permission").first()).toBeVisible();
    vars["CompanyName"] = await CorrPortalElem.First_Company_Name.first().textContent() || '';
    vars["Companyname"] = String(vars["CompanyName"]).trim();
    await expect(CorrPortalElem.First_Company_Name.first()).toContainText(vars["Companyname"]);
  } catch (error) {
    throw error;
  }
}
...
export async function stepGroup_Store_Company_Names_from_Company_Permissions(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  try {
    ...
    // }
    if (!(await CorrPortalElem.Standard_Flow_On_Button.isChecked())) {
      await CorrPortalElem.Standard_Flow_On_Button.click();
    }

    if (!(await CorrPortalElem.Chase_Direct_ON_button.isChecked())) {
      await CorrPortalElem.Chase_Direct_ON_button.click();
    }
    ...
    // await page.pause(); // Debugging pause to inspect the state before entering search criteria
    await CorrPortalElem.Search_Filter_Input_in_Customer_Permission.fill(vars["Company name 2"]);
    await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
...
export async function stepGroup_Selecting_the_multiple_Company_name_Creating_a_New_Map(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  Methods.getCurrentTimestamp(appconstants.DATE_FORMAT_SLASH, 'CurrentDate');/* format: dd/MM/yyyy/HH:mm:ss */;
  Methods.concatenate(appconstants.Testsigma_, vars['CurrentDate'], 'Create New Map');
  ...
  await CorrPortalElem.ThirdCompany_Checkbox.check();
...
export async function stepGroup_Add_Rule_For_Add_Condition_In_Rules_and_Actions(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.Operation_Dropdown.first().selectOption({ value: vars["Operation1"] });
  ...
  vars["RuleCondition"] = await CorrPortalElem.Operation_Dropdown.first().inputValue() || '';
...
export async function stepGroup_Uploading_the_File(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await expect(CorrPortalElem.Map_Headers_Button).toBeVisible();
  await CorrPortalElem.Map_Headers_Button.click();
  await CorrPortalElem.Save_and_Move_to_Next_Page.waitFor({ state: 'visible' });
  await expect(CorrPortalElem.This_action_will_save_the_changes_and_Move_to_Next_Page).toBeVisible();
  await CorrPortalElem.Proceed_with_Saving_Button.click();
...
export async function stepGroup_Add_Field_in_Enumeration_Mapping(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.Bid_Tape_Add_Field.type(Array.from({ length: 7 }, () => "abc".charAt(Math.floor(Math.random() * 3))).join(''));
...
export async function stepGroup_Editing_Header_Mapping(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  vars["UpdatedBidSampleName[HeaderMapping]"] = await CorrPortalElem.Custom_Header_Field.inputValue() || '';
  await CorrPortalElem.Update_Header_Button.click();
}
...
export async function stepGroup_Edition_In_Enumeration_Mapping(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await expect(CorrPortalElem.Chase_Values_Dropdown.locator('option:checked')).toHaveText(vars["EditedChaseValue[Enumeration Mapping]"]);
}
...
export async function stepGroup_Verification_Of_BidSampleNames_In_Header_Mapping_From_TDP(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  for (let i = 0; i < Number(vars['BidEnumValueCount']); i++) {
    ...
    log.info('Correspondent Chase Field Name: ' + vars["Correspondent Chase Field Name"]);
    await expect(CorrPortalElem.get_Individual_BidSample_Name(vars['count'])).toContainText(vars["Bid Sample Field Name"]);
    vars['BidSampleFieldName'] = vars['Bid Sample Field Name'];
    vars["ChaseValue"] = await CorrPortalElem.Mapped_Chase_Value_2(vars['BidSampleFieldName']).evaluate(el => { const s = el as HTMLSelectElement; return s.options[s.selectedIndex]?.text || ''; });
    expect(Methods.verifyString(vars['ChaseValue'], 'equals', vars['Correspondent Chase Field Name']));
...
export async function stepGroup_Creating_New_Header_In_Header_Mapping_Screen(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.Custom_Header_Field.pressSequentially(vars["CustomHeader"]);
...
export async function stepGroup_Save_Draft_Exit_Action_and_Navigate_from_new_map_to_enumerat(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.Save_Draft_Exit_Button.click();
  await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
  await CorrPortalElem.get_Bid_Map_Name_Field_In_Row(vars["CreateNewMap"]).waitFor({ state: 'visible' });
  await CorrPortalElem.get_Bid_Map_Name_Field_In_Row(vars["CreateNewMap"]).click();
  await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
  await CorrPortalElem.Map_Headers_Button.click();
  await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
...
export async function stepGroup_Creating_New_Bid_Mapping_For_search_action_based_on_the_rule(page: import('@playwright/test').Page, vars: Record<string, string>) {
  const CorrPortalElem = new CorrPortalPage(page);
  await stepGroup_Creation_Of_Bid_Map_Upto_Header_Mapping(page, vars, fileconstants.BID_QA_FILE_COMMON);
  await CorrPortalElem.Enumeration_Mapping_Button.click();
  await CorrPortalElem.Yes_Proceed_Button.click();
  await CorrPortalElem.Rules_and_Actions_Button.waitFor({ state: 'visible' });
  await CorrPortalElem.Rules_and_Actions_Button.click();
...
export async function stepGroup_Duplicating_Rule_In_Enumeration_Mapping(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  expect(String(vars["Rule Name Input Field_Duplicated"])).toBe(vars["Duplicated Rule Name"]);
...
export async function stepGroup_Verification_of_Rules_and_Action_Values_Before_EditingActive(page: import('@playwright/test').Page, vars: Record<string, string>) {
  const CorrPortalElem = new CorrPortalPage(page);
  await expect(CorrPortalElem.Rule_Name_Field).toHaveValue(vars["First Rule Name"]);
  ...
  await expect(CorrPortalElem.Add_Actions_Chase_Field_Name.locator('option:checked')).toHaveText(vars["ChaseFieldNameonAddActions"]);
...
export async function stepGroup_Adding_Actions_In_Rules_and_Actions_Screen(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.Chase_Field_Name.selectOption({ label: vars["Unique Chase Field Name"] });
  await CorrPortalElem.Chase_Value_Dropdown_Rules_Actions.click();
  await CorrPortalElem.Chase_Value_Search_field.fill(vars["Unique Chase Value"]);
  ...
  Methods.trimtestdata(vars["ChasevalueOnAddActions"], 'ChasevalueOnAddActions');
}
...
export async function stepGroup_Creating_Of_Bid_Maps_with_Standard(page: import('@playwright/test').Page, vars: Record<string, string>) {
  const CorrPortalElem = new CorrPortalPage(page);
  const helpers = new AddonHelpers(page, vars);
  const correspondentPortalPage = new CorrespondentPortalPage(page);
  // const testData: Record<string, string> = {}; // TODO: Load from test data profile
...
export async function stepGroup_Navigating_to_Customer_Permission_For_the_Chase_Direct_Compa(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await expect(page.getByText("Customer Permission").first()).toBeVisible();
  await CorrPortalElem.Search_Filter_Input_in_Customer_Permission.click();
  await CorrPortalElem.Search_Filter_Input_in_Customer_Permission.fill(vars["Company name 1"]);
  await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
  vars["CompanyName"] = await CorrPortalElem.Company_Name_ChaseDirect.first().textContent() || '';
  vars["Companyname"] = String(vars["CompanyName"]).trim();
  await expect(CorrPortalElem.Company_Name_ChaseDirect.first()).toContainText(vars["Companyname"]);
}
...
export async function stepGroup_Navigate_to_Customer_Permission_to_Fetch_First_Company_Name(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.CustomerPermission_Menu.click();
  await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
  // await expect(CorrPortalElem.Customer_Permission_text).toBeVisible();
  await CorrPortalElem.Search_Filter_Input_in_Customer_Permission.click();
  await CorrPortalElem.Search_Filter_Input_in_Customer_Permission.pressSequentially(vars["Company name 1"]);
  await CorrPortalElem.Spinner.waitFor({ state: 'hidden' });
  await expect(CorrPortalElem.First_Company_Name.first()).toBeVisible({ timeout: 3000 });
  vars["CompanyName"] = await CorrPortalElem.First_Company_Name.first().textContent() || '';
  vars["Companyname"] = String(vars["CompanyName"]).trim();
  await expect(CorrPortalElem.First_Company_Name.first()).toContainText(vars["Companyname"]);
}
...
export async function stepGroup_EditActions_In_CustomerPermission(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await CorrPortalElem.Company_Name_Action_Button_FirstRow.click();
  await CorrPortalElem.Edit_Permissions.waitFor({ state: 'visible' });
  // if (await CorrPortalElem.Standard_Flow_On_Button.notIsChecked()) /* Radio button Standard_Flow_On_Button is not selected */ {
  //   await CorrPortalElem.Standard_Flow_On_Button.click();
  // }
  // if (await CorrPortalElem.Chase_Direct_ON_button.notIsChecked()) /* Radio button Chase_Direct_ON_button is not selected */ {
  //   await CorrPortalElem.Chase_Direct_ON_button.click();
  // }
  // if (await CorrPortalElem.Update_Permissions_Button.isEnabled) /* Element Update Permissions Button is enabled */ {
  //   await CorrPortalElem.Update_Permissions_Button.click();
...
export async function stepGroup_Verify_that_the_Bid_Upload_Progress_Popup_has_All_statuses_w(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  await page.waitForTimeout(5000);
  log.info('All Rows Count: ' + vars['AllRowsCount']);
  vars["count"] = appconstants.ONE;
  while (parseFloat(String(vars["count"])) <= parseFloat(String(vars["AllRowsCount"]))) {
    log.info('Iteration: ' + vars['count']);
    vars["IndividualStatus"] = await CorrPortalElem.Individual_Status_On_Popup(vars['count']).textContent() || '';
    Methods.trimtestdata(vars['IndividualStatus'], 'IndividualStatus');
    expect(Methods.verifyString('Success , Warning', 'contains', vars['IndividualStatus']));
    Methods.performArithmetic(vars["count"], 'ADDITION', '1', 'count', 0);
  }
}
...
export async function stepGroup_Verifying_the_table_data_in_bid_request_details_from_tdp(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  const profileName = 'Bid Request Details Tables Data For ChaseDirect';
  const profile = testDataManager.getProfileByName(profileName);
  const dataList = profile?.data as Record<string, any>[];

  vars["TotalRowsCount"] = String(await CorrPortalElem.Rows_Count_Table_1.count());
  log.info('Total Rows Count: ' + vars['TotalRowsCount']);
  ...
  while (parseFloat(String(vars["RowsCount"])) <= parseFloat(String(vars["TotalRowsCount"]))) {
    ...
    while (parseFloat(String(vars["ColumnCount"])) <= parseFloat(String("7"))) {
      ...
      if (String(vars["ColumnCount"]) === String("1")) {
        vars['Loan Number'] = dataList[Number(vars['tdpcount'])]['Loan Number'];
        log.info('Loan Number: ' + vars['Loan Number']);
        await expect(CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount'])).toContainText(vars["Loan Number"]);
      } else if (String(vars["ColumnCount"]) === String("2")) {
        vars['Last Name'] = dataList[Number(vars['tdpcount'])]['Last Name'];
        log.info('Last Name: ' + vars['Last Name']);
        await expect(CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount'])).toContainText(vars["Last Name"]);
      } else if (String(vars["ColumnCount"]) === String("3")) {
        vars['LoanAmount'] = dataList[Number(vars['tdpcount'])]['LoanAmount'];
        log.info('LoanAmount: ' + vars['LoanAmount']);
        await expect(CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount'])).toContainText(vars["LoanAmount"]);
      } else if (String(vars["ColumnCount"]) === String("4")) {
        vars['Program'] = dataList[Number(vars['tdpcount'])]['Program'];
        log.info('Program: ' + vars['Program']);
        await expect(CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount'])).toContainText(vars["Program"]);
      } else if (String(vars["ColumnCount"]) === String("5")) {
        vars['Loan Status'] = dataList[Number(vars['tdpcount'])]['Loan Status'];
        log.info('Loan Status: ' + vars['Loan Status']);
        await expect(CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount'])).toContainText(vars["Loan Status"]);
      } else if (String(vars["ColumnCount"]) === String("6")) {
        vars["CellData"] = await CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount']).textContent() || '';
        if (String(vars["CellData"]).includes(String("+"))) {
          await CorrPortalElem.Footer_Queued_For_Date.click();
          await CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount']).hover();
          vars["CellDataPopup"] = await CorrPortalElem.Tool_Tip_Text.textContent() || '';
          Methods.concatenate(vars["CellDataPopup"], vars["CellData"], 'CellData');
          vars['Errors'] = dataList[Number(vars['tdpcount'])]['Errors'];
          log.info('Errors: ' + vars['Errors']);
          Methods.verifyString(vars['CellData'], 'contains', vars['Errors']);
        }
        else {
          vars['Error Description'] = dataList[Number(vars['tdpcount'])]['Error Description'];
          log.info('Error Description: ' + vars['Error Description']);
          await expect(CorrPortalElem.Individual_Cell_Data(vars['RowsCount'], vars['ColumnCount'])).toContainText(vars["Error Description"]);
        }
      }
    ...
    if (await bidRequestDetailsPage.Individual_PQ_Button(vars['RowsCount']).isEnabled()) {
      vars['PQ_Status UI'] = 'PQ Enabled';
    } else {
      vars['PQ_Status UI'] = 'PQ Disabled';
    }
    if (await bidRequestDetailsPage.Individual_PS_Button(vars['RowsCount']).isEnabled()) {
      vars['PS_Status UI'] = 'PS Enabled';
    } else {
      vars['PS_Status UI'] = 'PS Disabled';
    }
    log.info('PS Status UI: ' + vars['PS_Status UI']);
    log.info('PQ Status UI: ' + vars['PQ_Status UI']);
    vars['PQ Status'] = dataList[Number(vars['tdpcount'])]['PQ Status'];
    log.info('PQ Status: ' + vars['PQ Status']);
    Methods.verifyString(vars['PQ_Status'], 'contains', vars['PQ Status']);
    vars['PS Status'] = dataList[Number(vars['tdpcount'])]['PS Status'];
    log.info('PS Status: ' + vars['PS Status']);
    Methods.verifyString(vars['PS_Status'], 'contains', vars['PS Status']);
    Methods.performArithmetic(vars['tdpcount'], 'ADDITION', '1', 'tdpcount', 0);
    Methods.performArithmetic(vars['RowsCount'], 'ADDITION', '1', 'RowsCount', 0);
  }
}
...
export async function stepGroup_Verify_Footer_Submission_and_Queued_Date_For_Today_in_Bid_Re(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  const Methods = new AddonHelpers(page, vars);
  await expect(CorrPortalElem.Footer_Submission_Date).toContainText(vars["CurrentESTDate"]);
  Methods.addMinutesToDatetime(vars['CurrentEstTime'], appconstants.EST_TIME_FORMAT_HHMMA, 1, appconstants.EST_TIME_FORMAT_HHMMA, 'CurrentEstPlusOneMin');
  Methods.removeCharactersFromPosition(vars["CurrentEstPlusOneMin"], '6', '0', 'CurrentEstUnitAfterAddition');
  Methods.removeCharactersFromPosition(vars["CurrentEstPlusOneMin"], '0', '3', 'CurrentEstHourMinAfterAddition');
  Methods.subtractMinutesFromDatetime(vars['CurrentEstTime'], appconstants.EST_TIME_FORMAT_HHMMA, 1, appconstants.EST_TIME_FORMAT_HHMMA, 'CurrentEstMinusOneMin');
  Methods.removeCharactersFromPosition(vars["CurrentEstMinusOneMin"], '6', '0', 'CurrentEstTimeUnitMinus1');
  Methods.removeCharactersFromPosition(vars["CurrentEstTime"], '0', '3', 'CurrentEstHourMinMinus1');
  Methods.removeCharactersFromPosition(vars["CurrentEstTime"], '6', '0', 'CurrentEstTimeUnit');
  Methods.removeCharactersFromPosition(vars["CurrentEstTime"], '0', '3', 'CurrentEstHourMin');
  vars['Footer Submission Date'] = await CorrPortalElem.Footer_Submission_Date.textContent() || '';
  /* Verify that the element Footer Submission Date displays text CurrentEstHourMin*/
  if (String(vars['Footer Submission Date']).includes(String(vars['CurrentEstHourMin']))) {
    await expect(CorrPortalElem.Footer_Submission_Date).toContainText(vars["CurrentEstTimeUnit"]);
  }
  /* Verify that the element Footer Submission Date displays text CurrentEstHourMinMinus1 */
  else if (String(vars['Footer Submission Date']).includes(String(vars['CurrentEstHourMinMinus1']))) {
  ...
  Methods.subtractMinutesFromDatetime(vars['SelectedBatchTime'], appconstants.EST_TIME_FORMAT_HHMMA, Number(vars['BufferTime']), appconstants.EST_TIME_FORMAT_HHMMA, 'SelectedBatchTimeWithoutBuffer');
...
export async function stepGroup_Verifying_the_bidsample_to_bidtape_mapping_in_Enumpage_from_(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  while (parseFloat(String(vars["count1"])) <= parseFloat(String(vars["EnumFieldsCount"]))) {
    ...
    for (let i = 0; i < Number(vars["EnumFieldsCount"]); i++) {
      ...
      log.info('Enum Bid Sample Field Names: ' + vars["EnumBidSampleNames"]);
      vars['EnumBidTapeValues'] = dataList[i]['EnumBidTapeValues'];
      log.info('Enum Bid Tape Values: ' + vars["EnumBidTapeValues"]);
      vars["IndividualBidSampleName"] = await CorrPortalElem.get_Individual_Bid_Sample_Name_Enum_Page(vars['count1']).textContent() || '';
      expect(Methods.verifyString(vars['IndividualBidSampleName'], 'contains', vars['EnumBidSampleNames']));
      vars["ColumnHeader"] = vars["IndividualBidSampleName"];
      /* Element BidTapeFieldCountForBidField is not visible */
      if (!(await CorrPortalElem.BidTapeFieldCountForBidField(vars['ColumnHeader']).first().isVisible())) {
        log.info('BidTapeFieldCountForBidField is not visible');
        vars["IndividualBidTapeValue"] = "No BidTape";
      } else {
        vars["IndividualBidTapeValue"] = "Sample";
        vars["BidTapeCount"] = String(await CorrPortalElem.BidTapeFieldCountForBidField(vars['ColumnHeader']).count());
      ...
      expect(Methods.verifyString(vars['IndividualBidTapeValue'], 'contains', vars['EnumBidTapeValues']));
      Methods.MathematicalOperation(vars['count1'], '+', 1, 'count1');
    }
  }
}
...
export async function stepGroup_Verifying_the_Mapping_of_ChaseField_and_ChaseValues_in_Enum_(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  while (parseFloat(String(vars["count1"])) <= parseFloat(String(vars["ChaseFieldsCountEnum"]))) {
    ...
    for (let i = 0; i < Number(vars["ChaseFieldsCountEnum"]); i++) {
      log.info('Testdata Iteration: ' + vars["count1"]);

      vars['ChaseFieldName'] = dataList[i]['ChaseFieldName'];
      log.info('Chase Field Name: ' + vars["ChaseFieldName"]);

      vars['Chase Value'] = dataList[i]['Chase Value'];
      log.info('Chase Value: ' + vars["Chase Value"]);

      vars["IndividualChaseFieldName"] = await CorrPortalElem.Individual_Chase_Enum_Name(vars['count1']).textContent() || '';
      expect(Methods.verifyString(vars["ChaseFieldName"], 'contains', vars["IndividualChaseFieldName"]));

      if (!(await CorrPortalElem.ChaseValues_Corresponding_to_Chase_Field(vars['IndividualChaseFieldName']).first().isVisible())) {
        log.info('ChaseValues Corresponding to Chase Field is not visible');
        ...
        while (parseFloat(String(vars["count2"])) <= parseFloat(String(vars["ChaseValuesOfChaseFieldCount"]))) {
          log.info('Chase Values Of Chase Field Count: ' + vars["ChaseValuesOfChaseFieldCount"]);
          ...
          log.info('Tag Name: ' + vars["TagName"]);
          if (String(vars["TagName"]) === String("SELECT")) {
            vars["ChaseValue"] = await CorrPortalElem.Individual_ChaseValue_of_ChaseField(vars['IndividualChaseFieldName'], vars['count2']).evaluate(el => {
      ...
      expect(Methods.verifyString(vars['IndividualChaseValueofChaseField'], 'contains', vars['Chase Value']));
      Methods.MathematicalOperation(vars['count1'], '+', 1, 'count1');
    }
  }
}
...
export async function stepGroup_Deleting_Early_Config_Report_If_Present(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  if (vars['EarlyCloseConfigRowsText'].includes(appconstants.NO_RESULT_TEXT)) {
    ...
    if (await CorrPortalElem.Early_Close_Config_Rows.first().isVisible()) {
...
export async function stepGroup_Storing_Chase_Field_and_Chase_Value_from_Enum_Page_With_Mapp(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  while (parseFloat(String(vars["count1"])) <= parseFloat(String(vars["ChaseFieldsCountEnum"]))) {
    ...
    await CorrPortalElem.First_Checkbox_Enum.uncheck();
    vars["IndividualChaseFieldName"] = await CorrPortalElem.Individual_Chase_Enum_Name(vars['count1']).textContent() || '';
    log.info('Individual Chase Field Name: ' + vars["IndividualChaseFieldName"]);
    if (!(await enumerationMappingPage.get_ChaseValues_Corresponding_to_Chase_Field(vars["IndividualChaseFieldName"]).first().isVisible())) /* Element ChaseValues Corresponding to Chase Field is not visible*/ {
      ...
      while (parseFloat(String(vars["count2"])) <= parseFloat(String(vars["ChaseValuesOfChaseFieldCount"]))) {
        ...
        log.info('ChaseValue: ' + vars["ChaseValue"]);
        Methods.concatenateWithSpecialChar(vars["ChaseValue"], vars["IndividualChaseValueofChaseField"], ',', 'IndividualChaseValueofChaseField');
        Methods.performArithmetic(vars["count2"], 'ADDITION', '1', 'count2', 0);
      }
      testDataManager.updatePartialProfileDataByDataIndex(ProfileName, {
...
export async function stepGroup_Headers_Verification_in_Closed_List(page: import('@playwright/test').Page, vars: Record<string, string>) {
  ...
  while (parseFloat(String(vars["Count"])) <= parseFloat(String(vars["CountOfHeaders"]))) {
    vars["IndividualHeaderNameUI"] = await CorrPortalElem.Individual_Headers_Commitment_List(vars["Count"]).textContent() || '';
    Methods.trimtestdata(vars["IndividualHeaderNameUI"], "IndividualHeaderNameUI");
    Methods.splitStringByRegConditionWithPosition(vars["EntireHeadersExcel"], ",", vars["Count"], "IndividualHeaderNameExcel");
...
/**
 * Step Group: Selecting Second Enabled Batch Time If the Condition is failed in bid request details
 * Steps: 8
 */
export async function stepGroup_Selecting_Second_Enabled_Batch_Time_If_the_Condition_is_fail_BidReqDetails_Page(page: import('@playwright/test').Page, vars: Record<string, string>) {
  const CorrPortalElem = new CorrPortalPage(page);
  const BidRequestDetails = new BidRequestDetailsPage(page);
  const Methods = new AddonHelpers(page, vars);

  vars["EnabledTime"] = await CorrPortalElem.Enabled_Time.first().textContent() || '';
  Methods.trimtestdata(vars["EnabledTime"], 'EnabledTime');
  ...
  Methods.calculateTimeDifference(vars["EnabledTime"], vars['CurrentEstTime'], 'MINUTES', 'TimeDiff');
  log.info('Time difference of enabled time and current est time: ' + vars['TimeDiff']);
  if (String(vars["TimeDiff"]) >= String("4")) {
    await BidRequestDetails.Resubmit_Pricing_Dropdown_new.selectOption({ value: vars["EnabledTime"] });
  } else {
    vars["SecondEnabledTime"] = await CorrPortalElem.Second_Enabled_Time.textContent() || '';
    Methods.trimtestdata(vars["SecondEnabledTime"], 'SecondEnabledTime');
    log.info(`Second Enabled Time from UI: "${vars["SecondEnabledTime"]}"`);
    await BidRequestDetails.Resubmit_Pricing_Dropdown_new.selectOption({ value: vars["SecondEnabledTime"] });
  }
}