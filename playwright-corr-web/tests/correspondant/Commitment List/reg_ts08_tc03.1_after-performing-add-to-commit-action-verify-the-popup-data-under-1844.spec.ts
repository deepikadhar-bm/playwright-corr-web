...
test.describe('Commitment List - TS_2', () => {
  ...
  "UniqueRuleNameSearch": "TsSearchUniqueRuleName"
    ...
    await priceOfferedPage.Committed_CorrLoan.scrollIntoViewIfNeeded();
    ...
    while (parseFloat(String(vars["count"])) <= parseFloat(String(vars["ChaseFieldCountPopup"]))) {
      ...
      vars["ChaseFieldNamePopupTotalCommittedLoans"] = await bidRequestDetailsPage.Individual_Chase_Field_Name_Popup.textContent() || '';
      vars["ChaseValuePopupTotalCommittedLoans"] = await commitmentListPage.Individual_Chase_ValueTotal_Committed_Loans_Popup.textContent() || '';
      for (let dataIdx = parseInt(vars["count"]); dataIdx <= parseInt(vars["count"]); dataIdx++) {
        await expect(bidRequestDetailsPage.Individual_Chase_Field_Name_Popup).toContainText(testData["Chase Field Name"]);
        if (String(vars["ChaseValuePopupTotalCommittedLoans"]) === String("key_blank")) {
          expect(String(testData["Chase Field Value"])).toBe("Null");
        } else {
          await expect(commitmentListPage.Individual_Chase_ValueTotal_Committed_Loans_Popup).toContainText(testData["Chase Field Value"]);
        }
