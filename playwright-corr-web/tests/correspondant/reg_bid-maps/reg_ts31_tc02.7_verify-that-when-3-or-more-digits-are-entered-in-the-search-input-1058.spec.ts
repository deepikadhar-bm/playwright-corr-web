...
test.describe('Unassigned', () => {
  ...
  "UniqueRuleNameSearch": "TsSearchUniqueRuleName"
    ...
    await statusInactive2Page.SearchFilter_Fields.fill(testData["UniqueChaseValueSearch"]);
    ...
    await expect(bidmapsPage.Action_SearchFilter_Value_Text).toContainText(testData["UniqueChaseValueSearch"]);
    ...
    await spinnerPage.Spinner.waitFor({ state: 'hidden' });
    await expect(rulesAndActionsPage.Chase_Value_Rules_Actions).toContainText(testData["UniqueChaseValueSearch"]);
