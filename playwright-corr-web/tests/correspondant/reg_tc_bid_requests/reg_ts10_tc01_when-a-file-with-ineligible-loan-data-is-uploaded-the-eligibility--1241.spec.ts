...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  "Bid Enum Check(Error Description)": "enum field occupancy type is missing value or value is not in valid list [primary residence, secondary residence, investment (noo)]. path: search.criteria.propertyuse is missing value"
    ...
    if (String(testData["Eligibility Check(Error Column)"]).includes(String(vars["ErrorColumnData"]))) {
      await expect(bidRequestDetailsPage.Errors_Column).toContainText(testData["Eligibility Check(Error Column)"]);
    } else {
    ...
    await expect(bidRequestPage.Error_Description_ColumnBid_request_details).toContainText(testData["Eligibility Check(Error Description)"]);
    await expect(bidRequestDetailsPage.Loan_Status_Column).toContainText(testData["Eligibility Check(LoanStatus)"]);
