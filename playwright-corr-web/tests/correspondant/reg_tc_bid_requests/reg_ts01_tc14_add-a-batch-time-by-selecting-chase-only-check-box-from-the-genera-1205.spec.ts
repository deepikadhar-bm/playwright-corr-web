...
import { runPrereq_909 } from '../../../src/helpers/prereqs/prereq-909';
...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  test('REG_TS01_TC14_Add a batch time by selecting Chase only check box from the general settings module and verify that batch time value should be updated here under today\\\'s pricing return time value.', async ({ page }) => {
    ...
    vars["AddedBatchTime"] = await bidRequestPage.Added_Batch_2.textContent() || '';
    vars["PricingReturnTimeBuffer"] = await statusInactivePage.Pricing_Return_Time_Buffer.inputValue() || '';
    vars["AddedBatchTime"] = (() => {
