...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  "Bid Enum Check(Error Description)": "enum field occupancy type is missing value or value is not in valid list [primary residence, secondary residence, investment (noo)]. path: search.criteria.propertyuse is missing value"
    ...
    await correspondentPortalPage.Upload_File.setInputFiles(path.resolve(__dirname, 'test-data', "GEO-2.1-Failed_loan.xlsx"));
    ...
    await expect(bidRequestDetailsPage.Errors_Column).toContainText(testData["Geo Coding Failed flow(Errors))"]);
    await expect(bidRequestPage.Error_Description_ColumnBid_request_details).toContainText(testData["Geo Coding Failed flow(Error Description)"]);
    await expect(bidRequestDetailsPage.Loan_Status_Column).toContainText(testData["Geo Coding Failed flow(Loan Status)"]);
