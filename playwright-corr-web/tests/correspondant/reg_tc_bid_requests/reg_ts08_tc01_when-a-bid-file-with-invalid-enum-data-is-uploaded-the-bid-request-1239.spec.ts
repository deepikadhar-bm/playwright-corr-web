...
test.describe('REG_TC_Bid_Requests', () => {
  ...
  "Missing Headers(Error Message)": "Bid tape is missing values for following headers: Loan Purpose. Please verify the correct Map ID is selected or tape is formatted correctly."
    ...
    await correspondentPortalPage.Upload_File.setInputFiles(path.resolve(__dirname, 'test-data', "Bid_Enum_Check_UploadProgress.xlsx"));
    ...
    if (true) /* Element Continue Button(Upload Pop up) is disabled */ {
      await expect(bidRequestPage.Continue_ButtonUpload_Pop_up).toContainText("Continue");
    }
    ...
    if (String(testData["Bid Enum Check(Errors Column)"]).includes(String(vars["ErrorColumnData"]))) {
      await expect(bidRequestDetailsPage.Errors_Column).toContainText(testData["Bid Enum Check(Errors Column)"]);
    } else {
      await bidRequestDetailsPage.Errors_Column.hover();
      await expect(bidRequestDetailsPage.Error_Column_popup).toBeVisible();
      await expect(bidRequestDetailsPage.Error_Column_popup).toContainText(testData["Bid Enum Check(Errors Column)"]);
    }
    await expect(bidRequestPage.Error_Description_ColumnBid_request_details).toContainText(testData["Bid Enum Check(Error Description)"]);
    await expect(bidRequestDetailsPage.Loan_Status_Column).toContainText(testData["Bid Enum Check(Loan Status)"]);
